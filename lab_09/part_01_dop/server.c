...
int main(int argc, char *argv[])
{
	...
	while (TRUE)
	{
		...
		bytes = recvfrom(sock2, recv_buf, 100, 0, (struct sockaddr*)&rcvr_name, strlen(rcvr_name.sun_path) + sizeof(rcvr_name.sun_family));
		...
		if(bytes > 0)
		{
			...
			if(sendto(sock2, send_buf, strlen(send_buf)*sizeof(char), 0, (struct sockaddr*)&rcvr_name, strlen(rcvr_name.sun_path) + sizeof(rcvr_name.sun_family)) == -1 )
			{

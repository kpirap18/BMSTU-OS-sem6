...
int doPath(myfunc *func, char *fullpath, int depth)
{
    ...
    if (!S_ISDIR(statbuf.st_mode)) // файловый режим
    {
        ...
        func(fullpath, FTW_F, depth, statbuf.st_ino); // для отрисовки

        return SUCCESS; // ВОЗВРАТ если это не каталог
    }

    // если не файл, а каталог, то ++ каталоги
    func(fullpath, FTW_D, depth, statbuf.st_ino); // отрисовка
    stats.ndir++;
...
static int myFunc(const char *pathname, int type, int depth, ino_t i)
{
    ...
    if (type == FTW_D)
    {
        printf("    ├── %s (D) \t(inode %d) %s\n", pathname, (int)(i), RESET);
    }
    else
    {
        printf("    ├── %s \t(inode %d) %s\n", pathname, (int)(i), RESET);

...
static int limit = 20; //значение по умолчанию, его можно менять здесь
...
static void *ct_seq_start(struct seq_file *s, loff_t *pos) {
     ...
     //Выделяем память для числа, которое будет хранить наше растущее четное значение
...
static int __init ct_init(void) {
     proc_create("evens", 0666, NULL, &ct_file_ops);
     return 0;
}
